...
##### Results file should be in a specific format: more info will be added later but I copy-paste abd then use polish-commands to make it
...
#table$V10 = as.numeric(str_trim(table$V10))
table$V11 = as.numeric(str_trim(table$V11))

results=table
reformat <- function(results, experiment){
  results = results[,2:10]
  colnames(results)=c("dataset","tool","nga50","ng50","n50", "misassemblies", "fraction","time","memory","tool.order")
